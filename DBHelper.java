...
// vector holds the results.    
    public static Vector<String []> doQuery(String s) {
        String user = "jadrn028";
        String password = "storm";
        String database = "jadrn028";
...
// are closed, or a memory leak occurs in Tomcat.            
    ...
    public static String getSpecialQueryResultTable(Vector<String []> v) {
        StringBuffer toReturn = new StringBuffer();
       for(int i = 0;i< v.size();i++){
        toReturn.append(v.elementAt(i));
       }
        return toReturn.toString();
    }

     public static Vector<String []> doProductQuery(String s) {
        String user = "jadrn028";
        String password = "storm";
        String database = "jadrn028";
        String connectionURL = "jdbc:mysql://opatija:3306/" + database +
        ...
        try {
            ...
            while(resultSet.next()) {
                ...
                for(int i=0; i < numCols; i++)
                    tmp[i] = resultSet.getString(i+1)+"||";  // resultSet getString is 1 based
                    //tmp[numCols-1] = tmp[numCols-1].substring(0,tmp[numCols-1].length()-2);
